...
(function() {
    ...
    var CruncherCtrl = function($scope, $rootScope, $timeout) {
        ...
        var getOverallDamage = function(damage, hitModifiers, noSorting, type) {
            ...
            return {
                damage: result.result, overall: overallDamage,
        ...
        var getStatOfUnit = function(data, stat, slot) {
            ...
            enabledSpecials.forEach(function(special) {
                if (params.team[special.sourceSlot].unit.number + 1 == 3805 && special.specialType == "special") {
        ...
        var getFinalTap = function(params) {
            let computedFinalTap = 1;
            const finalTapEffects = [];
            finalTapEffects.push(...enabledEffects.filter((effect) => effect.hasOwnProperty("finalTap")));
            finalTapEffects.push(...enabledSpecials.filter(((special) => special.hasOwnProperty("finalTap"))));

            if (finalTapEffects.length === 0) {
                if(typeof window.finalTapCondition === "function") {
                    window.finalTapCondition = undefined;
                    $scope.notify({ text : "A condition on Final Tap has been disabled" });
                }
                window.finalTapValue = 0;
                return computedFinalTap;
            }

            for (const effect of finalTapEffects) {
                const cachedParams = getCachedParameters(effect.sourceSlot, effect.specialType);
                params.cached = cachedParams;
                const value = effect.finalTap(params);
                if (value == -1) {
                    // Buff not applied based on conditions
                    continue;
                } else {
                    if (typeof window.finalTapCondition === "function") {
                        if (window.finalTapCondition(params)) {
                            computedFinalTap += value;
                        }
                    } else {
                        window.finalTapCondition = effect.hasOwnProperty("finalTapCondition") ? effect.finalTapCondition : (() => true);
                        $scope.notify({
                            text : "A condition on Final Tap has been enabled"
                        });
                        // Buff applied and increment
                        computedFinalTap += value;
                    }
                }
            }

            // Final tap is limited to 200%
            computedFinalTap = Math.min(computedFinalTap, 3);
            window.finalTapValue = computedFinalTap;
            return computedFinalTap;
        }
        ...
        var applyChainAndBonusMultipliers = function(damage, modifiers, type) {
            ...
            chainSpecials.forEach(function(special) {
                ...
                var result = damage.map((x) => {
                    //Computing Final Tap boost
                    const y = x;
                    y.finalTap = getFinalTap(getParameters(x.position));
                    return y;
                });
                ...
                if ($scope.tdata.orderOverride.hasOwnProperty(type)) {
                    ...
                    result = result.sort((x, y) => {
                        // Calculate which one has the most damage with finalTap
                        return Math.floor(x.base * totalMultiplier([...x.multipliers, [x.finalTap, "finalTap"]])) - Math.floor(y.base * totalMultiplier([...y.multipliers, [y.finalTap, "finalTap"]]));
                    })
                }

                result = result.map((x, i) => {
                    // Only apply finalTap to the last of the chain
                    const y = x;
                    if (i === damage.length - 1) y.multipliers.push([y.finalTap, "finalTap"]);
                    delete y.finalTap;
                    return y;
                })

                var result = result.map(function(x, n) {
            ...
            for (let i = 0; i < currentResult.result.length; i++) {
                const result = currentResult.result[i];
